...
public class Session implements ISession {
    ...
    public Session(String host, int port) throws IOException {
        ...
        this.tSender = Thread.ofVirtual().name("Sender - IP : " + this.ip).unstarted(this.sender != null ? this.sender.setSocket(this.socket) : (this.sender = new Sender(this, this.socket)));
        this.tCollector = Thread.ofVirtual().name("Collector - IP : " + this.ip).unstarted(this.collector != null ? this.collector.setSocket(this.socket) : (this.collector = new Collector(this, this.socket)));
        this.tQueueHandler = Thread.ofVirtual().name("QueueHandler - IP : " + this.ip).unstarted(this.queueHandler);
    }

    public Session(Socket socket) {
        this.KEYS = "NgojcDev".getBytes();
        this.id = ID_INIT++;
        this.socket = socket;
        try {
            this.socket.setSendBufferSize(0x100000);
