...
import boss.list.NinjaAoTim;
import boss.list.RobotVeSi;
import boss.list.TrungUyThep;
import boss.list.TrungUyTrang;
import boss.list.TrungUyXanhLo;
import clan.Clan;
import map.Zone;
import mob.Mob;
...
public class RedRibbonHQ implements Runnable {
    ...
    private void init() {
        ...
        for (Zone zone : this.zones) {
            ...
            int dame = totalHp / 20;
            int hp = totalDamage * 50;

            if (zone.map.mapId == 59) {
                try {
                    int bossDamage = Math.min(dame, 200_000_000);
                    int bossMaxHealth = Math.min(hp, 2_000_000_000);
            ...
            if (zone.map.mapId == 62) {
                try {
                    int bossDamage = Math.min((dame * 110 / 100), 200_000_000);
                    int bossMaxHealth = Math.min((hp * 110 / 100), 2_000_000_000);
            ...
            if (zone.map.mapId == 55) {
                try {
                    int bossDamage = Math.min((dame * 115 / 100), 200_000_000);
                    int bossMaxHealth = Math.min((hp * 115 / 100), 2_000_000_000);
            ...
            if (zone.map.mapId == 54) {
                try {
                    int bossDamage = Math.min((dame * 120 / 100), 200_000_000);
                    int bossMaxHealth = Math.min((hp * 120 / 100), 2_000_000_000);
            ...
            if (zone.map.mapId == 57) {
                try {
                    int bossDamage = Math.min((dame * 130 / 100), 200_000_000);
                    int bossMaxHealth = Math.min((hp * 130 / 100), 2_000_000_000);
                    for (int i = 0; i < 4; i++) {
    ...
    public ItemMap NR(Zone zone) {
        ...
        int nr = Util.isTrue(1, 500) ? Util.nextInt(15, 18) : Util.nextInt(17, 20);
    ...
    public void updateHPDame() {
        ...
        for (Zone zone : this.zones) {
            for (Mob mob : zone.mobs) {
                ...
                mob.point.maxHp = (int) (totalDame * mob.tempId < 2_000_000_000 ? totalDame * mob.tempId
                        : 2_000_000_000);
