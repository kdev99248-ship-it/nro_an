...
import boss.list.TrungUyXanhLo_BDKB;
import clan.Clan;
import map.TrapMap;
import map.Zone;
import mob.Mob;
    ...
    private void init() {
        ...
        for (Zone zone : this.zones) {
            ...
            if (zone.map.mapId == 135 || zone.map.mapId == 136 || zone.map.mapId == 137) {
                ...
                for (int i = 0; i < mobs.size(); i++) {
                    ...
                    if (((i == 5 || i == 10) && zone.map.mapId == 135) || (i == 5 && zone.map.mapId == 136)
                        ...
                        mob.lvMob = 1;
                        mob.point.dame = Math.min(level * 31 * 50 * mob.tempId * 10, 2_000_000_000);
                        mob.point.maxHp = Math.min(level * 3107 * 50 * mob.tempId * 10, 2_000_000_000);
                        mob.hoiSinh();
                        mob.hoiSinhMobPhoBan();
                    } else {
                        mob.lvMob = 0;
                        mob.point.dame = Math.min(level * 31 * 50 * mob.tempId, 2_000_000_000);
                        mob.point.maxHp = Math.min(level * 3107 * 50 * mob.tempId, 2_000_000_000);
                        mob.hoiSinh();
                        mob.hoiSinhMobPhoBan();
                    }
                }
            ...
            if (zone.map.mapId == 137) {
                try {
                    int bossDamage = (200000 * level);
                    int bossMaxHealth = (200000000 * level);
                    bossDamage = Math.min(bossDamage, 200000000);
                    bossMaxHealth = Math.min(bossMaxHealth, 2000000000);
                    boss = new TrungUyXanhLo_BDKB(
                            zone,
                            level,
                            (int) bossDamage,
                            (int) bossMaxHealth);
