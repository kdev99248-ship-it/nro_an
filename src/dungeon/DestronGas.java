    ...
    public void update() {
        if (isOpened) {
            ...
            if (allCharactersDead && !callBoss) {
                try {
                    int bossDamage = (10000 * level);
                    int bossMaxHealth = (15000000 * level);
        ...
        for (Zone zone : this.zones) {
            ...
            for (int i = 0; i < mobs.size(); i++) {
                ...
                if (((i == 5 || i == 10) && zone.map.mapId == 149) || ((i == 5 || i == 10 || i == 15)
                    ...
                    mob.lvMob = 1;
                    mob.point.dame = Math.min(level * 31 * 5 * mob.tempId * 10, 2_000_000_000);
                    mob.point.maxHp = Math.min(level * 3107 * 5 * mob.tempId * 10, 2_000_000_000);
                    mob.hoiSinh();
                    mob.hoiSinhMobPhoBan();
                } else {
                    mob.lvMob = mob.tempId == 76 ? 1 : 0;
                    mob.point.dame = Math.min(level * 31 * 5 * mob.tempId, 2_000_000_000);
                    mob.point.maxHp = Math.min(level * 3107 * 5 * mob.tempId, 2_000_000_000);
                    mob.hoiSinh();
                    mob.hoiSinhMobPhoBan();
                }
            }
