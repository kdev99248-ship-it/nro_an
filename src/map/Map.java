...
import mob.GaChinCua;
import mob.GauTuongCuop;
import mob.Hirudegarn;
import mob.NguaChinLmao;
import mob.Piano;
import mob.RobotBaoVe;
import mob.VoiChinNga;
import mob.VuaBachTuoc;
...
public class Map implements Runnable {
    ...
    public Map(int mapId, String mapName, byte planetId,
        ...
        this.planetName = mapId == 84 || mapId == 154 ? "Hành Trình Nro" : Service.gI().get_HanhTinh(planetId);
        this.tileId = tileId;
        this.bgId = bgId;
        this.bgType = bgType;
        this.type = type;
        ...
        while (true) {
            try {
                ...
                if (this.zones.size() > 1) {
                    this.zones.parallelStream().forEach(zone -> {
                        try {
                            zone.update();
                        } catch (Exception e) {
                            Logger.logException(Map.class, e, "Lỗi update zone " + zone.zoneId);
                        }
                    });
                } else {
                ...
                long sleepTime = Math.max(100 - elapsed, 5);
