...
public class Zone {
    ...
    private volatile int countItemAppeaerd = 0;
    
    // Thread-safe method để generate item map ID
    public synchronized int getNextItemMapId() {
        int nextId = countItemAppeaerd++;
        if (countItemAppeaerd >= 2000000000) {
            countItemAppeaerd = 0;
        }
        return nextId;
    }
    ...
    private int mobUpdateIndex = 0;
    private int itemUpdateIndex = 0;
    private int npcUpdateIndex = 0;
    private static final int ENTITIES_PER_CYCLE = 3;
    ...
    public void update() {
        try {
            udMobSmooth();
            udItemSmooth(); 
            udNonInteractiveNPCSmooth();
            
        } catch (Exception e) {
            Logger.logException(Zone.class, e, "Lỗi update zone");
            try {
                udMob();
                udItem();
                udNonInteractiveNPC();
            } catch (Exception e2) {
                Logger.logException(Zone.class, e2, "Lỗi fallback update");
            }
        }
    }
    
    private void udMobSmooth() {
        if (this.mobs.isEmpty()) return;
        
        try {
            int processed = 0;
            int totalMobs = this.mobs.size();
            
            for (int i = 0; i < totalMobs && processed < ENTITIES_PER_CYCLE; i++) {
                int index = (mobUpdateIndex + i) % totalMobs;
                try {
                    if (index < this.mobs.size()) {
                        mobs.get(index).update();
                        processed++;
                    }
                } catch (Exception e) {
                    Logger.logException(Zone.class, e, "Lỗi update mob " + index);
                }
            }
            
            mobUpdateIndex = (mobUpdateIndex + processed) % Math.max(totalMobs, 1);
            
        } catch (Exception e) {
            Logger.logException(Zone.class, e, "Lỗi udMobSmooth");
        }
    }
    
    private void udItemSmooth() {
        if (this.items.isEmpty()) return;
        
        // Synchronize để tránh concurrent modification
        synchronized(this.items) {
            try {
                ...
                for (int i = 0; i < totalItems && processed < ENTITIES_PER_CYCLE; i++) {
                    int index = (itemUpdateIndex + i) % totalItems;
                    try {
                        if (index < this.items.size()) {
                            ItemMap item = this.items.get(index);
                            if (item != null && item.itemTemplate != null) {
                                item.update();
                                processed++;
                            } else {
                                ...
                                if (index <= itemUpdateIndex) {
                                    itemUpdateIndex = Math.max(0, itemUpdateIndex - 1);
                                }
                                totalItems = this.items.size();
                                // Adjust loop to account for removed item
                                i--;
                            }
                        }
                    } catch (Exception e) {
                        Logger.logException(Zone.class, e, "Lỗi update item " + index);
                ...
                if (totalItems > 0) {
                    itemUpdateIndex = (itemUpdateIndex + processed) % totalItems;
                } else {
                    itemUpdateIndex = 0;
                }
                
            } catch (Exception e) {
                Logger.logException(Zone.class, e, "Lỗi udItemSmooth");
            }
        }
    }
    
    private void udNonInteractiveNPCSmooth() {
        if (this.nonInteractiveNPCs.isEmpty()) return;
        
        try {
            int processed = 0;
            int totalNPCs = this.nonInteractiveNPCs.size();
            
            for (int i = 0; i < totalNPCs && processed < ENTITIES_PER_CYCLE; i++) {
                int index = (npcUpdateIndex + i) % totalNPCs;
                try {
                    if (index < this.nonInteractiveNPCs.size()) {
                        Player pl = this.nonInteractiveNPCs.get(index);
                        if (pl != null && pl.zone != null) {
                            pl.update();
                            processed++;
                        }
                    ...
                    Logger.logException(Zone.class, e, "Lỗi update npc " + index);
                }
            }
            
            npcUpdateIndex = (npcUpdateIndex + processed) % Math.max(totalNPCs, 1);
            
        } catch (Exception e) {
            Logger.logException(Zone.class, e, "Lỗi udNonInteractiveNPCSmooth");
        }
    }
    ...
    public synchronized ItemMap getItemMapByItemMapId(int itemId) {
    ...
    public synchronized ItemMap getItemMapByTempId(int tempId) {
        for (ItemMap item : this.items) {
    ...
    public synchronized List<ItemMap> getItemMapsForPlayer(Player player) {
    ...
    public synchronized void pickItem(Player player, int itemMapId) {
    ...
    private void infoPlayer(Player plReceive, Player plInfo) {
        ...
        try {
            ...
            msg.writer().writeInt(plInfo.nPoint != null ? plInfo.nPoint.hp : 1);
            msg.writer().writeInt(plInfo.nPoint != null ? plInfo.nPoint.hpMax : 1);
...
//            msg.writer().writeInt(plInfo.nPoint.hpMax);
        ...
        Service.gI().sendEffPlayer(plInfo);
    ...
    // Utility method để cleanup invalid items
    public synchronized void cleanupInvalidItems() {
        if (this.items.isEmpty()) return;
        
        try {
            java.util.Iterator<ItemMap> iterator = this.items.iterator();
            ...
            Logger.logException(Zone.class, e, "Error cleaning up invalid items");
        }
    }
    
    // Method để get items count thread-safe
    public synchronized int getItemsCount() {
        return this.items.size();
    }
    
    // Method để check if zone has too many items (performance optimization)
    public boolean hasExcessiveItems() {
        return getItemsCount() > 1000; // Threshold có thể adjust
    }
    
    // Getter methods thay thế cho Lombok annotations
