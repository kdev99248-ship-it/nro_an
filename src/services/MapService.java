...
public class MapService {
    ...
    public Zone getZone(int mapId) {
        ...
        // Tối ưu hóa load balancing cho zones
        Zone bestZone = null;
        int minPlayers = Integer.MAX_VALUE;
        
        for (Zone zone : map.zones) {
            int playerCount = zone.getNumOfPlayers();
            if (playerCount < zone.maxPlayer && playerCount < minPlayers) {
                minPlayers = playerCount;
                bestZone = zone;
            }
        }
        
        // Nếu tất cả zones đều full, trả về zone đầu tiên (fallback)
    ...
    public boolean isMapAll(int mapId) {
        return mapId >= 0 && mapId <= 178;
    }
    ...
    public boolean isMapMaQuai(int mapId) {
        return mapId == 184 || mapId == 185;
    }
