...
public class Service {
    ...
    public void sendMessAllPlayerInMap(Zone zone, Message msg) {
        ...
        // Tạo snapshot để tránh ConcurrentModificationException
        Player[] playerArray;
        try {
            playerArray = players.toArray(new Player[0]);
        } catch (Exception e) {
            msg.dispose();
            return;
        }
        ...
        for (Player pl : playerArray) {
            if (pl != null && pl.getSession() != null && pl.isPl()) {
                try {
                    ...
                    // Log nhưng không dừng việc gửi cho players khác
                    Logger.logException(Service.class, e, "Error sending message to player " + pl.id);
                }
            }
        }
        msg.cleanup();
    ...
    public void sendThongBaoFromAdmin(Player player, String text) {
        sendBigMessage(player, 14240, text);
    }

    public void sendThongBaoFromBill(Player player, String text) {
        sendBigMessage(player, 5067, text);
    }
    ...
    public void dropItemMap(Zone zone, ItemMap item) {
        if (zone == null || item == null || item.itemTemplate == null) {
            return;
        }
        Message msg = null;
        try {
            ...
            Logger.logException(Service.class, e, "Error in dropItemMap");
        } finally {
            if (msg != null) {
                msg.cleanup();
            }
    ...
    public void dropItemMapForMe(Player player, ItemMap item) {
        if (player == null || item == null || item.itemTemplate == null) {
            return;
        }
        Message msg = null;
        try {
            ...
            Logger.logException(Service.class, e, "Error in dropItemMapForMe");
        } finally {
            if (msg != null) {
                msg.cleanup();
            }
    ...
    // Batch processing method để drop nhiều items cùng lúc - Optimized version
    public void dropMultipleItemsMap(Zone zone, java.util.List<ItemMap> items) {
        if (zone == null || items == null || items.isEmpty()) {
            return;
        }

        // Filter valid items
        java.util.List<ItemMap> validItems = new java.util.ArrayList<>();
        ...
        // Batch send - tạo một message chứa nhiều items (nếu protocol hỗ trợ)
        // Hoặc gửi tuần tự với optimization
        List<Player> players;
        try {
            players = zone.getPlayers();
            if (players == null || players.isEmpty()) {
        ...
        // Gửi từng item với optimized loop
        for (ItemMap item : validItems) {
            Message msg = null;
            try {
                msg = new Message(68);
                msg.writer().writeShort(item.itemMapId);
                ...
                // Gửi đến tất cả players trong snapshot
                for (Player pl : playerArray) {
                    if (pl != null && pl.getSession() != null && pl.isPl()) {
                        try {
                            pl.sendMessage(msg);
                        } catch (Exception e) {
                            // Continue với players khác
                        }
                    }
                }
            } catch (Exception e) {
                Logger.logException(Service.class, e, "Error in batch drop item");
            } finally {
                if (msg != null) {
                    msg.cleanup();
                }
    ...
    // Optimized method cho việc drop items với delay để tránh spam
    private static final java.util.Map<Zone, Long> lastDropTime = new java.util.concurrent.ConcurrentHashMap<>();
    private static final long DROP_DELAY_MS = 50; // 50ms delay giữa các drops

    public void dropItemMapWithDelay(Zone zone, ItemMap item) {
        if (zone == null || item == null) {
            return;
        }

        Long lastTime = lastDropTime.get(zone);
        long currentTime = System.currentTimeMillis();

        if (lastTime != null && (currentTime - lastTime) < DROP_DELAY_MS) {
            // Delay để tránh spam
            try {
                Thread.sleep(DROP_DELAY_MS - (currentTime - lastTime));
        ...
        dropItemMap(zone, item);
        lastDropTime.put(zone, System.currentTimeMillis());
    }
    ...
    public void sendEffPlayer(Player pl) {
        ...
        // Các slot cần check: 12 = Danh Hiệu, 11 = Chân Mệnh
        int[] checkSlots = {12, 11};

        for (int slot : checkSlots) {
            if (pl.inventory.itemsBody.size() > slot) {
                var item = pl.inventory.itemsBody.get(slot);
                if (item != null && item.isNotNullItem()) {
                    sendEffAllPlayer(pl, item.template.part);
                    // Logger.log("SEND EFF slot " + slot + " : " + item.template.part);
                }
            }
        }
    }
