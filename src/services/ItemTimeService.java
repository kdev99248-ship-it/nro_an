...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isUseCoBonLa) {
            sendItemTime(player, 13618, (int) (TIME_EAT_CO_BON_LA - (System.currentTimeMillis() - player.itemTime.lastTimeUseCoBonLa)) / 1000);
        }
        ...
        if (player.itemTime.isUseAnDanh2) {
            sendItemTime(player, 10717,
                    (int) ((ItemTime.TIME_AN_DANH_2 - (System.currentTimeMillis() - player.itemTime.lastTimeAnDanh2)) / 1000));
        }
        ...
        if (player.itemTime.isUsePetBuff) {
            sendItemTime(player, 13540,
                    (int) ((TIME_PET_BUFF - (System.currentTimeMillis() - player.itemTime.lastTimeUsePetBuff)) / 1000));
        }
    }
    ...
    public void useCoBonLa(Player pl, Item item) {
        if (pl.itemTime.isUseCoBonLa) {
            Service.gI().sendThongBao(pl, "Bạn đang dùng vật phẩm này rồi");
            return;
        }
