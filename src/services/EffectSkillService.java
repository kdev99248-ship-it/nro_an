...
public class EffectSkillService {
    ...
    public void removeHuytSao(Player p) {
        p.effectSkill.tiLeHPHuytSao = 0;
        sendEffectPlayer(p, p, TURN_OFF_EFFECT, HUYT_SAO_EFFECT);
        int hpMax = Math.max(1, p.nPoint.hpMax);
        if (p.nPoint.hp > hpMax) {
            p.nPoint.setHp(hpMax);
        }
        Service.gI().point(p);
