...
import boss.list.SuperBroly;
import control.boss.Rival;
...
public class SkillService {
    ...
    public void useSkillAttack(Player player, Player plTarget, Mob mobTarget) {
            ...
            if (player.isPet) {
                ...
                if (player.nPoint.stamina > 0) {
                    if (player.charms.tdDeoDai < System.currentTimeMillis()) {
                        player.nPoint.numAttack++;
                        if (player.nPoint.numAttack == 100) {
                            player.nPoint.numAttack = 0;
                            player.nPoint.stamina--;
        ...
        switch (player.playerSkill.skillSelect.template.id) {
            case Skill.KAIOKEN: // kaioken
                int hpUse = Util.maxIntValue(player.nPoint.hpMax / 100 * 10);
        ...
        switch (player.playerSkill.skillSelect.template.id) {
                        ...
                        if (!player.isBoss) {
                            ...
                            for (Player pl : playersMap) {
                                ...
                                if (!pl.isBoss && pl.gender != ConstPlayer.NAMEC
                                    ...
                                    pl.nPoint.setHP(
                                            pl.nPoint.hp - ((pl.nPoint.hpMax * 10 / 100) < pl.nPoint.hp
            ...
            case Skill.TU_SAT:
                if (!player.playerSkill.prepareTuSat) {
                    ...
                    long elapsedTime = currentTime - player.playerSkill.lastTimePrepareTuSat;

                    if (player.playerSkill.tuSatCheatAttempts > 0 &&
                            currentTime - player.playerSkill.lastTuSatCheatTime > 60000) {
                        player.playerSkill.tuSatCheatAttempts = 0;
                    }

                    long minimumDelay = 2000;

                    if (!player.isBoss && !player.isPet && elapsedTime < minimumDelay) {
                        player.playerSkill.skillSelect.lastTimeUseThisSkill = System.currentTimeMillis();
                        player.playerSkill.prepareTuSat = false;

                        player.playerSkill.tuSatCheatAttempts++;
                        player.playerSkill.lastTuSatCheatTime = currentTime;

                        if (player.playerSkill.tuSatCheatAttempts >= 3) {
                            if (player.playerSkill.tuSatCheatAttempts == 3) {
                                Logger.warningln("[ANTICHEAT] SEVERE TU_SAT cheat: " + player.name +
                                        " | Attempts: " + player.playerSkill.tuSatCheatAttempts +
                                        " | Time: " + elapsedTime + "ms");
                            }

                            player.playerSkill.skillSelect.lastTimeUseThisSkill = currentTime + 10000;
                        } else {
                            if (player.playerSkill.tuSatCheatAttempts == 1) {
                                Logger.warningln("[ANTICHEAT] TU_SAT cheat detected: " + player.name +
                                        " | Time: " + elapsedTime + "ms (required: " + minimumDelay + "ms)");
                            }
                        }
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        ...
        int dameAttack = plAtt.nPoint.getDameAttack(false);
