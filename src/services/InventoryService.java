...
//                    return new int[]{io.optionTemplate.id, io.param};
    ...
    private void handleOption210(Item item) {
        for (int i = 0; i < item.itemOptions.size(); i++) {
            ...
            if (io.optionTemplate.id == 210) {
                ...
                int[] allOptions = {10, 14, 108, 94, 108, 16, 111, 109, 97, 94, 101, 5, 106};
                ...
                for (int option : selectedOptions) {
                    ...
                    if (option == 5) {                    // 5% - 20%
                        ...
                        newParam = 5 + (int) (Math.random() * (20 - 5 + 1));   // [5..20]
                    } else if (option == 109) {           // cố định 10%
                        newParam = 10;
                    } else if (option == 10) {            // 5% - 18%
                        ...
                        newParam = 10 + (int) (Math.random() * 16);             // [10..25]
                    } else if (option == 106) {
                        newParam = 0;
                    }
    ...
    public void tradeKeo(Player player, int sl, boolean all) {
        ...
        if (biNgo == null || biNgo.quantity < 10) {
            Service.gI().sendThongBao(player, "Không đủ Bí ngô để đổi");
            return;
        }

        Item keoMu = ItemService.gI().createNewItem((short) 1726);
        keoMu.itemOptions.add(new ItemOption(30, 0));

        if (all) {
            int slTemp = biNgo.quantity / 10;
            if (slTemp <= 0) {
                Service.gI().sendThongBao(player, "Cần ít nhất 10 Bí ngô để đổi một kẹo mù");
                return;
            }

            keoMu.quantity = slTemp;
            InventoryService.gI().subQuantityItemsBag(player, biNgo, slTemp * 10);
            InventoryService.gI().addItemBag(player, keoMu);
            InventoryService.gI().sendItemBags(player);
            Service.gI().sendThongBao(player, "Đổi thành công " + slTemp + " kẹo mù!");
            return;
        }

        if (biNgo.quantity < sl * 10) {
            Service.gI().sendThongBao(player, "Cần x" + (10 * sl) + " Bí ngô");
            return;
        }

        keoMu.quantity = sl;
        InventoryService.gI().subQuantityItemsBag(player, biNgo, sl * 10);
        InventoryService.gI().addItemBag(player, keoMu);
        InventoryService.gI().sendItemBags(player);
        Service.gI().sendThongBao(player, "Đổi thành công " + sl + " kẹo mù!");
    }

    public void tradeThiep(Player player, int sl, boolean all) {
        // Kiểm tra túi trống
        if (InventoryService.gI().getCountEmptyBag(player) == 0) {
            Service.gI().sendThongBao(player, "Cần ít nhất 1 ô hành trang trống");
            return;
        ...
        // Tìm item trong túi
        Item keoMu = InventoryService.gI().findItemBag(player, 1726); // Kẹo mù
        Item thiep = InventoryService.gI().findItemBag(player, 1117); // Thiệp

        // Kiểm tra tồn tại item
        if (keoMu == null || keoMu.quantity < 30) {
            Service.gI().sendThongBao(player, "Cần ít nhất 30 Kẹo mù để đổi");
            return;
        }
        if (thiep == null || thiep.quantity < 5) {
            Service.gI().sendThongBao(player, "Cần ít nhất 5 Thiệp để đổi");
            return;
        }

        // Tạo hòm Halloween (ID 1116)
        Item homHalloween = ItemService.gI().createNewItem((short) 1116);
        homHalloween.itemOptions.add(new ItemOption(93, 3));
        homHalloween.itemOptions.add(new ItemOption(30, 0));

        if (all) {
            // Tính số lượng tối đa có thể đổi
            int slTheoKeo = keoMu.quantity / 30;
            int slTheoThiep = thiep.quantity / 5;
            int slDoi = Math.min(slTheoKeo, slTheoThiep);

            if (slDoi <= 0) {
                Service.gI().sendThongBao(player, "Không đủ nguyên liệu để đổi");
                return;
            }

            homHalloween.quantity = slDoi;
            // Trừ vật phẩm nguyên liệu
            InventoryService.gI().subQuantityItemsBag(player, keoMu, slDoi * 30);
            InventoryService.gI().subQuantityItemsBag(player, thiep, slDoi * 5);
            InventoryService.gI().addItemBag(player, homHalloween);
            InventoryService.gI().sendItemBags(player);
            Service.gI().sendThongBao(player, "Đổi thành công " + slDoi + " Hòm Halloween!");
            return;
        }

        // Nếu chỉ đổi số lượng chỉ định (sl)
        if (keoMu.quantity < sl * 30 || thiep.quantity < sl * 5) {
            Service.gI().sendThongBao(player, "Cần x" + (sl * 30) + " Kẹo mù và x" + (sl * 5) + " Thiệp");
            return;
        }

        homHalloween.quantity = sl;
        InventoryService.gI().subQuantityItemsBag(player, keoMu, sl * 30);
        InventoryService.gI().subQuantityItemsBag(player, thiep, sl * 5);
        InventoryService.gI().addItemBag(player, homHalloween);
        InventoryService.gI().sendItemBags(player);
        Service.gI().sendThongBao(player, "Đổi thành công " + sl + " Hòm Halloween!");
    }

    public void tradeTuiMu(Player player, int sl, boolean all) {
        ...
        // Tìm item nguyên liệu
        Item keoMu = InventoryService.gI().findItemBag(player, 1726); // Kẹo Mù
        Item thiep = InventoryService.gI().findItemBag(player, 1117); // Thiệp

        // Kiểm tra đủ nguyên liệu
        if (keoMu == null || keoMu.quantity < 40) {
            Service.gI().sendThongBao(player, "Cần ít nhất 40 Kẹo mù để đổi Túi Mù");
            return;
        }
        if (thiep == null || thiep.quantity < 10) {
            Service.gI().sendThongBao(player, "Cần ít nhất 10 Thiệp để đổi Túi Mù");
            return;
        }

        // Tạo vật phẩm Túi Mù (id 1728)
        Item tuiMu = ItemService.gI().createNewItem((short) 1728);
        tuiMu.itemOptions.add(new ItemOption(93, 3));
        tuiMu.itemOptions.add(new ItemOption(30, 0)); // ví dụ thêm option rỗng

        if (all) {
            // Đổi toàn bộ có thể
            int slTheoKeo = keoMu.quantity / 40;
            int slTheoThiep = thiep.quantity / 10;
            int slDoi = Math.min(slTheoKeo, slTheoThiep);

            if (slDoi <= 0) {
                Service.gI().sendThongBao(player, "Không đủ nguyên liệu để đổi Túi Mù");
                return;
            }

            tuiMu.quantity = slDoi;
            InventoryService.gI().subQuantityItemsBag(player, keoMu, slDoi * 40);
            InventoryService.gI().subQuantityItemsBag(player, thiep, slDoi * 10);
            InventoryService.gI().addItemBag(player, tuiMu);
            InventoryService.gI().sendItemBags(player);
            Service.gI().sendThongBao(player, "Đổi thành công " + slDoi + " Túi Mù!");
            return;
        }

        // Nếu đổi theo số lượng chỉ định
        if (keoMu.quantity < sl * 40 || thiep.quantity < sl * 10) {
            Service.gI().sendThongBao(player, "Cần x" + (sl * 40) + " Kẹo mù và x" + (sl * 10) + " Thiệp");
            return;
        }

        tuiMu.quantity = sl;
        InventoryService.gI().subQuantityItemsBag(player, keoMu, sl * 40);
        InventoryService.gI().subQuantityItemsBag(player, thiep, sl * 10);
        InventoryService.gI().addItemBag(player, tuiMu);
        InventoryService.gI().sendItemBags(player);
        Service.gI().sendThongBao(player, "Đổi thành công " + sl + " Túi Mù!");
    }
