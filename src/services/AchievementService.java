...
import item.Item;
import mob.Mob;
import utils.Template.AchievementTemplate;
    ...
    public void confirmAchievement(Player player, byte select) {
        ...
        if (player.getSession().actived) {
            ...
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
                ...
                Item item = ItemService.gI().createNewItem((short) 190, money * 100);
                InventoryService.gI().addItemBag(player, item);
                InventoryService.gI().sendItemBags(player);
                Service.gI().sendMoney(player);
                Service.gI().sendThongBao(player, "Bạn vừa nhận được " + money + " Hồng ngọc.");
                Service.gI().sendThongBao(player, "Bạn vừa nhận được " + money * 100 + " vàng.");
            ...
            Service.gI().sendThongBao(player, TextServer.KHONG_HO_TRO);
        }
    }
