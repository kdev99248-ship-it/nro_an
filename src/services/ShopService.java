...
public class ShopService {
    ...
    public void opendShop(Player player, String tagName, boolean allGender) {
        if (tagName.equals("ITEMS_LUCKY_ROUND")) {
            ...
            return;
        } else if (tagName.equals("ITEMS_MAIL")) {
            openMailBox(player, tagName, player.inventory.itemsMail);
            return;
        }
        try {
    ...
    public void takeItem(Player player, byte type, int tempId) {
        ...
        if (tagName.equals("ITEMS_LUCKY_ROUND")) {
            ...
            getItemFromMailBox(player, player.inventory.itemsMail, type, tempId);
            return;
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        if (itemTempId == 934) {
            ...
            InventoryService.gI().sendItemBags(player);
            Service.gI().sendMoney(player);
            Service.gI().sendThongBao(player, "Bạn vừa mua 10 mảnh hồn bông tai ");
            return;
        }
    ...
    private void openMailBox(Player player, String tagName, List<Item> items) {
        if (items == null) {
            return;
        }
        player.idMark.setTagNameShop(tagName);
    ...
    private void getItemFromMailBox(Player player, List<Item> items, byte type, int index) {
        if (items == null) {
            return;
        }
        if (index < 0 || index >= items.size()) {
        ...
        switch (type) {
            ...
            case 2: // nhận hết
                ...
                InventoryService.gI().sendItemBags(player);
                if (countReceived > 0) {
                    Service.gI().sendThongBao(player, "Đã nhận " + countReceived + " thư");
                } else {
                    Service.gI().sendThongBao(player, "Hành trang đã đầy");
                }
                break;
        }
        openMailBox(player, player.idMark.getTagNameShop(), items);
    }
