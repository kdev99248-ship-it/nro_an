...
public class NPoint {
    ...
    private void setPointWhenWearClothes() {
        ...
        if (this.player.isPl() && this.player.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2
            ...
            for (Item item : this.player.inventory.itemsBag) {
                if (item.isNotNullItem() && item.template.id == 921 || item.template.id == 1121 || item.template.id == 1165) {
    ...
    private void setHpMax() {
        ...
        if (!this.player.isPet || (this.player.isPet && ((Pet) this.player).status != Pet.FUSION)) {
            if (this.player.effectSkill.tiLeHPHuytSao != 0) {
                hpMax += (hpMax / 100 * this.player.effectSkill.tiLeHPHuytSao);
            }
        }
        // Xử lý chibi
    ...
    private void setMpMax() {
        ...
        if (this.player.isPet && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA
            ...
            switch (((Pet) this.player).typePet) {
                case 1 ->// pet mabư
                        this.mpMax += (this.mpMax / 100 * 50);
                case 2 ->// pet berus
                        this.mpMax += (this.mpMax / 100 * 300);
                case 3 ->// pet pic
                        this.mpMax += (this.mpMax / 100 * 200);
                case 4 ->// pet black
                        this.mpMax += (this.mpMax / 100 * 400);
                case 5 ->// pet fide
                        this.mpMax += (this.mpMax / 100 * 100);
                case 6 ->// pet kid bư
                        this.mpMax += (this.mpMax / 100 * 200);
                case 7 ->// pet xên
                        this.mpMax += (this.mpMax / 100 * 150);
                case 8 ->// pet kid beerus
                        this.mpMax += (this.mpMax / 100 * 250);
                default -> {
                }
            }
        }
    ...
    private void setDame() {
        int dame = this.dameg + this.dameAdd;
        ...
        if (this.player.effectSkill.isMonkey) {
            if (!this.player.isPet || (this.player.isPet && ((Pet) this.player).status != Pet.FUSION)) {
                ...
                dame += (dame / 100 * percent);
            }
        }

        // Sức đánh đẹp
        dame += (dame / 100 * tlSexyDame);

        // Xử lý giảm dame
        dame -= (dame / 100 * tlSubSD);
    ...
    public void addHp(int hp) {
        if (hp > 0) {
            int potentialHp = this.hp + hp;
            if (potentialHp > this.hpMax) {
                this.hp = this.hpMax;
            } else {
                this.hp = Math.min(potentialHp, 2000000000);
            }
        }
    }
    ...
    public void setHp(int hp) {
        if (hp < 0) {
            ...
            this.hp = Math.min(hp, 2_000_000_000);
        }
    }
    ...
    public void setMp(int mp) {
        if (mp < 0) {
            ...
            this.mp = Math.min(mp, 2_000_000_000);
        }
    }
    ...
    public int getDameAttack(boolean isAttackMob) {
        ...
        switch (skillSelect.template.id) {
            ...
            case Skill.LIEN_HOAN:
                ...
                percentDameSkill = skillSelect.damage;
                // Kích hoạt +100% sát thương khi có 5 món set Liên Hoàn (option 233/234)
            ...
            case Skill.QUA_CAU_KENH_KHI:
                ...
                int dameqckk = (hpmob * 2 / 100) + (hppl * 2 / 100) + this.dame;

                if (this.player.setClothes.kirin == 5) {
                    dameqckk = dameqckk * 12 / 10; // Giảm từ x2 xuống x1.2
                }
    ...
    public long calSucManhTiemNang(long tiemNang) {
        if (power < getPowerLimit()) {
            ...
            if (this.player.isPet) {
                ...
                // Xử lý bùa x2 tiềm năng đệ tử
                if (((Pet) this.player).master.itemTime != null && ((Pet) this.player).master.itemTime.isUsePetBuff) {
                    tiemNang *= 2;
                }
            }
    ...
    public long getPowerLimit() {
        switch (limitPower) {
            ...
            case 9:
                return 90010000000L; // Đã mở giới hạn lần 9
            case 10:
                return 190010000000L; // Đã mở giới hạn lần 9
            default:
                return 0;
        }
    }
    ...
    public int getHpMpLimit() {
        switch (limitPower) {
            ...
            case 9:
                return 575000; // Đã mở giới hạn lần 9
            case 10:
                return 575000; // Đã mở giới hạn lần 9
            default:
                return 0;
    ...
    public int getDameLimit() {
        switch (limitPower) {
            ...
            case 9:
                return 25500; // Đã mở giới hạn lần 9
            case 10:
                return 25500; // Đã mở giới hạn lần 9
            default:
                return 0;
    ...
    public short getDefLimit() {
        switch (limitPower) {
            ...
            case 9:
                return 2000; // Đã mở giới hạn lần 9
            case 10:
                return 2000; // Đã mở giới hạn lần 9
            default:
                return 0;
    ...
    public byte getCritLimit() {
        switch (limitPower) {
            case 0:
                return 5; // Chưa mở giới hạn
            case 1:
                return 6; // Đã mở giới hạn lần 1
            case 2:
                return 7; // Đã mở giới hạn lần 2
            case 3:
                return 8; // Đã mở giới hạn lần 3
            case 4:
                return 9; // Đã mở giới hạn lần 4
            case 5:
                return 10; // Đã mở giới hạn lần 5
            case 6:
                return 10; // Đã mở giới hạn lần 6
            case 7:
                return 10; // Đã mở giới hạn lần 7
            case 8:
                return 10; // Đã mở giới hạn lần 8
            case 9:
                return 10; // Đã mở giới hạn lần 9
            case 10:
                return 10; // Đã mở giới hạn lần 9
            default:
                return 0;
    ...
    public void powerUp(long power) {
        // Xử lý bùa x2 sức mạnh đệ tử
        if (this.player.isPet && ((Pet) this.player).master.itemTime != null && ((Pet) this.player).master.itemTime.isUsePetBuff) {
            power *= 2;
        }
        this.power += power;
