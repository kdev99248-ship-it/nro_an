...
/*
 * @Author: NgojcDev
 * @Description: Nâng cấp đệ tử Kidbu
 */
...
public class NangDeKidbu {

    private static final int MANH_KIDBU_ID = 1760;   // Mảnh Kidbu
    ...
    private static final int TRUNG_DE_KIDBU_ID = 1763; // Trứng đệ Kidbu

    // Số lượng cần thiết
    private static final int REQUIRED_MANH_KIDBU = 500;
    private static final int REQUIRED_MANH_XEN = 500;
    private static final int REQUIRED_MANH_FIDE = 500;
    private static final int REQUIRED_TRUNG_MABU = 20;
    private static final int REQUIRED_DA_NGU_SAC = 30;

    // Tỉ lệ thành công
    private static final float SUCCESS_RATE = 60.0f;

    public static void showInfoCombine(Player player) {
        if (player.combineNew.itemsCombine.size() != 5) {
            CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                    "Cần đúng 5 loại vật phẩm:\n- 500 Mảnh Kidbu\n- 500 Mảnh Xên\n- 500 Mảnh Fide\n- 20 Trứng Mabư\n- 30 Đá ngũ sắc", "Đóng");
            return;
        }
        ...
        if (manhKidbu == null || manhXen == null || manhFide == null || trungMabu == null || daNguSac == null) {
            CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
        ...
        String errorMsg = "";
        if (manhKidbu.quantity < REQUIRED_MANH_KIDBU) {
            errorMsg += "Còn thiếu " + (REQUIRED_MANH_KIDBU - manhKidbu.quantity) + " Mảnh Kidbu\n";
        }
        ...
        String infoMsg = "|2|Nâng cấp đệ tử Kidbu\n|0|"
                + "Cần:\n"
                + "|1|~ " + REQUIRED_MANH_KIDBU + " Mảnh Kidbu\n"
                ...
                + "|2|Kết quả: Trứng đệ Kidbu";

        CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.MENU_START_COMBINE,
                infoMsg, "Nâng cấp", "Từ chối");
    }

    public static void nangDeKidbu(Player player) {
        if (player.combineNew.itemsCombine.size() != 5) {
            return;
        }
        ...
        if (manhKidbu == null || manhXen == null || manhFide == null || trungMabu == null || daNguSac == null
                || manhKidbu.quantity < REQUIRED_MANH_KIDBU
        ...
        if (Util.isTrue(SUCCESS_RATE, 100)) {
            ...
            InventoryService.gI().subQuantityItemsBag(player, manhKidbu, REQUIRED_MANH_KIDBU);
            ...
            Item trungDeKidbu = ItemService.gI().createNewItem((short) TRUNG_DE_KIDBU_ID, 1);
            InventoryService.gI().addItemBag(player, trungDeKidbu);

            CombineService.gI().sendEffectSuccessCombine(player);
            Service.gI().sendThongBao(player, "Chúc mừng! Bạn đã nâng cấp thành công và nhận được Trứng đệ Kidbu");
        } else {
