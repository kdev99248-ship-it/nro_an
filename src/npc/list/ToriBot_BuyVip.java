...
import author_ngojc.QuaToriBot;
import npc.Npc;
...
import utils.Util;
public class ToriBot_BuyVip extends Npc {

    public ToriBot_BuyVip(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        ...
        if (canOpenNpc(player)) {
            if (this.mapId == 0 || this.mapId == 7 || this.mapId == 14) {
                this.createOtherMenu(player, 100, "|7| Trong thời gian mùa 10 diễn ra\n"
                        + "( từ " + DragonRun.TIME_VIP_START + " đến hết " + DragonRun.TIME_VIP_END + ")\n"
            ...
            switch (player.idMark.getIndexMenu()) {
                ...
                case 223 -> { // Kích Hoạt VIP1
                    if (select == 0) {
                        ...
                        if (player.getSession().vnd >= 500_000) {
                ...
                case 224 -> { // Kích Hoạt VIP2
                    if (select == 0) {
                        ...
                        if (player.getSession().vnd >= 1_000_000) {
                ...
                case 225 -> { // Kích Hoạt VIP3
                    if (select == 0) {
                        ...
                        if (player.getSession().vnd >= 3_000_000) {
                            if (InventoryService.gI().getCountEmptyBag(player) < 8) {
                                this.npcChat(player, "cần 8 ô trống hành trang");
                                return;
                            }
                            ...
                            // Trừ điểm và subvip
                            PlayerDAO.subvnd(player, 3_000_000);
                ...
                case 226 -> { // Kích Hoạt VIP4
                    if (select == 0) {
                        ...
                        if (player.getSession().vnd >= 5_000_000) {
                            ...
                            // Trừ điểm và subvip
                            PlayerDAO.subvnd(player, 5_000_000);
