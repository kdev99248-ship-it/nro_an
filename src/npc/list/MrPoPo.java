...
public class MrPoPo extends Npc {
    ...
    public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
            if (this.mapId == 0) {
                String baseMessage = "Thượng Đế vừa phát hiện ra 1 loại khí đang âm thầm\nhủy diệt mọi mầm sống trên Trái Đất,\nnó được gọi là Destron Gas.\nTa sẽ đưa các cậu đến nơi ấy, các cậu đã sẵn sàng chưa?";
                
                // Thêm thông báo về ngày mở nếu không phải chủ nhật
                if (!TimeUtil.isSunday()) {
                    baseMessage += "\n\n[LƯU Ý: Khí Gas Hủy Diệt chỉ mở vào Chủ Nhật!]";
                }
            ...
            if (this.mapId == 0) {
                if (player.idMark.isBaseMenu()) {
                    switch (select) {
                        ...
                        case 3 -> {
                            // Kiểm tra ngày chủ nhật trước khi cho phép tham gia
                            if (!TimeUtil.isSunday()) {
                                NpcService.gI().createTutorial(player, tempId, this.avartar,
                                        "Khí Gas Hủy Diệt chỉ mở vào Chủ Nhật!\nHãy quay lại vào ngày Chủ Nhật.");
                                return;
                            }
