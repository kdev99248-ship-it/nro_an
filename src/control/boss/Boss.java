...
public class Boss extends Player implements IBoss {
                    ...
                    if (this.parentBoss == null) {
                        ...
                        if (this.isZone01SpawnDisabled && this.zone.map.zones.size() > 2) {
                            ...
                            if (zoneid < this.zone.map.zones.size()) {
                                ...
                                int randomZone = Util.nextInt(0, this.zone.map.zones.size() - 1);
                                this.zone = this.zone.map.zones.get(randomZone);
                            }
                        }
    ...
    protected void notifyJoinMap() {
        if (canSendNotify()) {
            ...
            Logger.logln("~> " + message + " Khu " + this.zone.zoneId);

        }
    }

    private boolean canSendNotify() {
        return !(this.zone.map.mapId == 140
            ...
            if (!prepareBom) {
                ...
                while (prepareBom) {
                    if (Util.canDoWithTime(lastTime, 2500)) {
                        setDie(this);
                        die(plAtt);
                        int dame = Boss.this.nPoint.hpMax;
