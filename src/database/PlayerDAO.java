...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            String itemsMail = dataArray.toJSONString();
            dataArray.clear();
            ...
            int[] skillsArr = gender == 0 ? new int[]{0, 1, 6, 9, 10, 20, 22, 19} : gender == 1 ? new int[]{2, 3, 7, 11, 12, 17, 18, 19} : new int[]{4, 5, 8, 13, 14, 21, 23, 19};
            ...
            dataArray.clear();
            String dataTopupRewards = dataArray.toJSONString();
            dataArray.clear();

            AlyraManager.executeUpdate("insert into player" + "(account_id, name, head, gender, have_tennis_space_ship, clan_id, " + "data_inventory, data_location, data_point, data_magic_tree, items_body, " + "items_bag, items_box, items_box_lucky_round, items_daban, items_mail, friends, enemies, data_intrinsic, data_item_time," + "data_task, data_mabu_egg, data_charm, skills, skills_shortcut, pet," + "data_black_ball, data_side_task, BoughtSkill, topup_rewards, spent_tv) " + "values ()", userId, name, hair, gender, 0, -1, inventory, location, point, magicTree, itemsBody, itemsBag, itemsBox, itemsBoxLuckyRound, itemsDaBan, itemsMail, friends, enemies, intrinsic, itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, dataBoughtSkill, dataTopupRewards, 0);
            Logger.success("{/} - Create New Player");
            return true;
        } catch (Exception e) {
            Logger.logException(PlayerDAO.class, e, "{/} - Erorr\n\n");
            return false;
        }
    }
    ...
    public static void updatePlayer(Player player) {
        if (player != null && player.idMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                // data items mail
                for (Item item : player.inventory.itemsMail) {
                    JSONArray opt = new JSONArray();
                    if (item.isNotNullItem()) {
                        dataItem.add(item.template.id);
                        dataItem.add(item.quantity);
                        ...
                        dataItem.add(options.toJSONString());
                        dataItem.add(item.content != null ? item.content : ""); // Lưu tag
                    } else {
                        dataItem.add(-1);
                        dataItem.add(0);
                        dataItem.add(opt.toJSONString());
                        dataItem.add(""); // Empty content cho null item
                    }
                    dataItem.add(item.createTime);
                    dataArray.add(dataItem.toJSONString());
                    dataItem.clear();
                ...
                dataArray.add(player.itemTime.isUseCoBonLa ? (ItemTime.TIME_EAT_CO_BON_LA - (System.currentTimeMillis() - player.itemTime.lastTimeUseCoBonLa)) : 0);
                ...
                // topup rewards
                for (String reward : player.topupRewards) {
                    dataArray.add(reward);
                }
                String topupRewards = dataArray.toJSONString();
                dataArray.clear();
                ...
                String query = "update player set head = ?, have_tennis_space_ship = ?, " + "clan_id = ?, data_inventory = ?, data_location = ?, data_point = ?, data_magic_tree = ?, " + "items_body = ?, items_bag = ?, items_box = ?, items_box_lucky_round = ?, items_daban = ?, items_mail = ?, friends = ?, " + "enemies = ?, data_intrinsic = ?, data_item_time = ?, data_task = ?, data_mabu_egg = ?, pet = ?, " + "data_black_ball = ?, data_side_task = ?, data_charm = ?, skills = ?, skills_shortcut = ?, notify = ?, " + "baovetaikhoan = ?, data_card = ?, lasttimepkcommeson = ?, bandokhobau = ?, doanhtrai = ?, conduongrandoc = ?, masterDoesNotAttack = ?, " + "nhanthoivang = ?, ruonggo = ?, sieuthanthuy = ?, vodaisinhtu = ?, rongxuong = ?, data_item_event = ?, data_luyentap = ?, data_clan_task = ?, data_vip = ?, " + "rank = ?, data_achievement = ?, giftcode = ?, topup_rewards = ?, BoughtSkill = ?, LearnSkill = ?, spent_tv = ?, point_boss = ?, hlw = ? where id = ?";

                AlyraManager.executeUpdate(query, player.head, player.haveTennisSpaceShip, (player.clan != null ? player.clan.id : -1), inventory, location, point, magicTree, itemsBody, itemsBag, itemsBox, itemsBoxLuckyRound, itemsDaBan, itemsMail, friend, enemy, intrinsic, itemTime, task, mabuEgg, pet, dataBlackBall, sideTask, charm, skills, skillShortcut, player.notify, dataBVTK, dataCard, player.lastPkCommesonTime, dataBDKB, player.lastTimeJoinDT, dataCDRD, player.doesNotAttack, dataNhanThoiVang, dataRuongGo, dataSieuThanThuy, dataVoDaiSinhTu, player.lastTimeShenronAppeared, dataItemEvent, dataLuyenTap, clanTask, dataVip, player.superRank.rank, achievement, giftCode, topupRewards, BoughtSkill, LearnSkill, player.spentTv, player.pointBoss, player.diemhalloween, player.id);
                ...
                Logger.logException(PlayerDAO.class, e, "{/} - Error\n\n" + player.name);
            }
        }
    }
    ...
    public static boolean subvnd(Player player, int num) {
        ...
        try {
            boolean success = AlyraManager.executeUpdate("update account set vnd = vnd - ? where id = ?", num, player.getSession().userId) > 0;
            if (success) {
    ...
    public static boolean addvip(Player player, int num) {
        ...
        try {
            boolean success = AlyraManager.executeUpdate("update account set vip = vip - ? where id = ?", num, player.getSession().userId) > 0;
            if (success) {
    ...
    public static boolean subcash(Player player, int num) {
        try {
            boolean success = AlyraManager.executeUpdate("update account set cash = cash - ? where id = ?", num, player.getSession().userId) > 0;
            if (success) {
    ...
    public static boolean addcash(Player player, int num) {
        try {
            boolean success = AlyraManager.executeUpdate("update account set cash = cash + ? where id = ?", num, player.getSession().userId) > 0;
            if (success) {
                player.getSession().cash += num;
    ...
    public static boolean addGoldBar(Player player, int num) {
        try {
            boolean success = AlyraManager.executeUpdate("update account set thoi_vang = thoi_vang + ? where id = ?", num, player.getSession().userId) > 0;
            if (success) {
                player.getSession().goldBar += num;
            }
            return success;
        } catch (Exception e) {
            Logger.logException(PlayerDAO.class, e, "Lỗi update thoi_vang " + player.name);
            return false;
        }
    }

    public static boolean subGoldBar(Player player, int num) {
        if (player.getSession().goldBar < num) {
            return false;
        }
        try {
            boolean success = AlyraManager.executeUpdate("update account set thoi_vang = thoi_vang - ? where id = ?", num, player.getSession().userId) > 0;
    ...
    public static boolean activateAccount(Player player) {
        try {
            boolean success = AlyraManager.executeUpdate("update account set active = 1 where id = ?", player.getSession().userId) > 0;
            if (success) {
                player.getSession().actived = true;
            }
            return success;
        } catch (Exception e) {
            Logger.logException(PlayerDAO.class, e, "Lỗi activate account " + player.name);
            return false;
        }
    }
    ...
    public static boolean createPlAo(int userId, String name, byte gender, int hair) {
        try {
            ...
            AlyraManager.executeUpdate("insert into player" + "(account_id, name, head, gender, have_tennis_space_ship, clan_id, " + "data_inventory, data_location, data_point, data_magic_tree, items_body, " + "items_bag, items_box, items_box_lucky_round, items_daban, items_mail, friends, enemies, data_intrinsic, data_item_time," + "data_task, data_mabu_egg, data_charm, skills, skills_shortcut, pet," + "data_black_ball, data_side_task) " + "values ()", userId, name, hair, gender, 0, -1, inventory, location, point, magicTree, itemsBody, itemsBag, itemsBox, itemsBoxLuckyRound, itemsDaBan, itemsMail, friends, enemies, intrinsic, itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask);
    ...
    public static void updateHlw(Player player) {
        try {
            AlyraManager.executeQuery("update player set hlw = hlw where player.id = ?", player.diemhalloween, player.id);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
