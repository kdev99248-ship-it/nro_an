...
import utils.Template.AchievementQuest;
...
public class NDVSqlFetcher {
    ...
    public static Player login(MySession session, AntiLogin al) {
        ...
        Player plInGame;
        try {
            rs = AlyraManager.executeQuery("select * from account where username = ? and password = ?", session.uu,
                    session.pp);
            if (rs.first()) {
                ...
                if (rs.getBoolean("ban")) {
                    ...
                    if (secondsPass < Manager.SECOND_WAIT_LOGIN) {
                        ...
                        rs = AlyraManager.executeQuery("select * from player where account_id = ? limit 1",
                                session.userId);
                        if (!rs.first()) {
                            ...
                            if ((player = loadPlayer(rs, false)) != null) {
                                ...
                                AlyraManager.executeUpdate("update account set last_time_login = '"
    ...
    private static Player loadPlayer(AlyraResultSet rs, boolean isOffline) throws Exception {
        ...
        try {
            ...
            // data items mail
            dataArray = (JSONArray) JSONValue.parse(rs.getString("items_mail"));
            if (dataArray != null) {
                for (Object o : dataArray) {
                    Item item;
                    JSONArray dataItem = (JSONArray) JSONValue.parse(o.toString());
                    short tempId = Short.parseShort(String.valueOf(dataItem.get(0)));
                    if (tempId != -1) {
                        item = ItemService.gI().createNewItem(tempId, Integer.parseInt(String.valueOf(dataItem.get(1))));
                        JSONArray options = (JSONArray) JSONValue
                        ...
                        // Load tag (content) nếu có
                        if (dataItem.size() > 3) {
                            String content = String.valueOf(dataItem.get(3));
                            if (!content.isEmpty() && !content.equals("null")) {
                                item.content = content;
                            }
                        }
                        player.inventory.itemsMail.add(item);
                    }
                }
                dataArray.clear();
            }
            ...
            if (dataArray.size() > 13) {
                timeEatCoBonLa = Integer.parseInt(String.valueOf(dataArray.get(13)));
            }
            ...
            if (dataArray.size() > 23) {
                timePetBuff = Integer.parseInt(String.valueOf(dataArray.get(23)));
            }
            ...
            player.itemTime.lastTimeUsePetBuff = System.currentTimeMillis() - (ItemTime.TIME_PET_BUFF - timePetBuff);
            ...
            player.itemTime.isUseCoBonLa = timeEatCoBonLa != 0;
            player.itemTime.isUseMayDo2 = timeMayDo2 != 0;
            player.itemTime.isUsePetBuff = timePetBuff != 0;
            ...
            // Topup Rewards
            try {
                dataArray = (JSONArray) JSONValue.parse(rs.getString("topup_rewards"));
                for (Object reward : dataArray) {
                    player.topupRewards.add((String) reward);
                }
                dataArray.clear();
            } catch (Exception e) {
            }
            ...
            try {
                player.spentTv = rs.getInt("spent_tv");
            } catch (Exception e) {
                player.spentTv = 0;
            }

            try {
                player.pointBoss = rs.getInt("point_boss");
            } catch (Exception e) {
                player.pointBoss = 0;
            }

            try {
                player.diemhalloween = rs.getInt("hlw");
            } catch (Exception e) {
                player.diemhalloween = 0;
            }
